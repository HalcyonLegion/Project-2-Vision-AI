...
const FashionFinderPage = ({ darkMode }) => {
  ...
  const getClothingInfo = async (description) => {
    try {
      const response = await axios.post('https://api.openai.com/v1/chat/completions', {
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a Fashion Expert who provides Clothing Pairing Suggestions based on the description of the clothing item. You will make suggestions for both Men and Women, and structure your response as follows: For Men: , For Women: .',
          },
          {
            role: 'user',
            content: `Please provide some clothing suggestions or pairings for ${description}. Please also suggest some suitable color combinations.`,
          },
        ],
      }, {
        headers: {
      ...
      const data = response.data;
      setClothingInfo(data.choices[0].message.content);
      setIsLoading(false);
    } catch (error) {
      console.error('Error:', error);
    }
    ...
    <div className={`min-h-screen py-6 flex flex-col justify-center sm:py-12 ${darkMode ? 'bg-gray-900' : 'bg-white'}`}>
      ...
      <div className="relative py-3 sm:max-w-xl sm:mx-auto">
        ...
        <div className="relative px-4 py-10 bg-white shadow-lg rounded-3xl sm:p-20">
          ...
          <div>
            <p className="mb-4">
              Upload a clothing image to get an AI-Powered Outfit Suggestion.
            </p>
            ...
            {clothingInfo && (
              <div>
                ...
                <div className="whitespace-pre-wrap break-words">{clothingInfo}</div>
              </div>
            )}
          </div>
...
export default FashionFinderPage;